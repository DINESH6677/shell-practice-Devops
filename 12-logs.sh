...
if [ $? -ne 0 ]; then
    ...
    echo -e " $W Mysql is already installed so....$N $Y skipping $N" | tee -a $logs_file
fi
...
if [ $? -ne 0 ]; then
    ...
    echo -e " $W Nginx is already installed so....$N $Y skipping $N" | tee -a $logs_file
fi
...
if [ $? -ne 0 ]; then
    ...
    echo -e " $W Python3 is already installed so....$N $Y skipping $N" | tee -a $logs_file
fi